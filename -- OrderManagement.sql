...
--
-- Host: localhost:3306
-- Generation Time: Oct 04, 2021 at 05:21 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`order_management`@`%` SQL SECURITY DEFINER VIEW `orderStatus`  AS SELECT `o`.`orderNumber` AS `orderNumber`, `o`.`status` AS `status`, sum((`d`.`quantityOrdered` * `d`.`priceEach`)) AS `total` FROM (`orders` `o` join `orderdetails` `d` on((`o`.`orderNumber` = `d`.`orderNumber`))) GROUP BY `o`.`orderNumber` ;
